...
def run_validation(sub_select):
    ...
    ax = axes[1, 0]
    ax.set_title(f'(C)  Magnitude — Arterial Compliance Pulse (HR = {hr_p:d} BPM)')
    add_koro(ax)
    ...
    ax = axes[1, 1]
    ax.set_title(f'(D)  Phase — Physical Arterial Displacement (HR = {hr_p:d} BPM)')
    add_koro(ax)

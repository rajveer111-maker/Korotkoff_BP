...
def robust_phase_unwrap_mean(iq):
    phase_unwrap = np.unwrap(np.angle(iq))
    dphi = np.diff(phase_unwrap)
    carrier_offset = np.mean(dphi)  # MATHEMATICALLY GUARANTEES 0 MEAN
...
phase_clean = robust_phase_unwrap_mean(iq)
...
print(f"Mean-based Displacement: range [{np.min(displacement_mm):.4f}, {np.max(displacement_mm):.4f}] mm")
print(f"Mean-based Filtered Heart Rate Pulse (hr_sig): range [{np.min(hr_sig):.6f}, {np.max(hr_sig):.6f}] mm")

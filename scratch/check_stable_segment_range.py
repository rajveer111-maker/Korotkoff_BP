...
# Bandpass filter from koro_rf_vs_stethoscope.py:
sos_h = butter(4, [0.5, 3.0], btype='band', fs=FS_RF, output='sos')
dh = sosfiltfilt(sos_h, phase_raw) * SCALE

# Segment 10 to 20 seconds
t_stable = dh[int(10 * FS_RF):int(20 * FS_RF)]

print(f"dh stable segment (10-20s): range [{np.min(t_stable):.6f}, {np.max(t_stable):.6f}] mm")
print(f"dh stable segment peak-to-peak: {np.max(t_stable) - np.min(t_stable):.6f} mm")

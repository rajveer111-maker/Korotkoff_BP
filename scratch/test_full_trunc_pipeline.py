...
# Truncate raw signals to start from 15 seconds to end
start_idx = int(15 * FS_RF)
i_trunc = i_raw[start_idx:]
q_trunc = q_raw[start_idx:]

# Run pipeline on truncated deflation data only
...
print(f"Truncated displacement: range [{np.min(displacement_mm):.4f}, {np.max(displacement_mm):.4f}] mm")
...
print(f"Truncated Filtered Heart Rate Pulse: range [{np.min(hr_sig):.6f}, {np.max(hr_sig):.6f}] mm")

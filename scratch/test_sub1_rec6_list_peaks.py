...
phi_vel = np.append(np.diff(sosfiltfilt(sos_vk, phi_clean))*FS_RF, 0.0)
...
# Normalize envelopes in the window
mask_koro = (t_rf >= K_ON) & (t_rf <= K_OFF)
t_koro = t_rf[mask_koro]
rf_koro_n = (rf_env[mask_koro] - np.min(rf_env[mask_koro])) / (np.max(rf_env[mask_koro]) - np.min(rf_env[mask_koro]) + 1e-20)
...
print("RF Peak Times (s):")
for idx, p in enumerate(peaks_rf):
    print(f"  RF Peak {idx+1}: {t_koro[p]:.3f} s")

print("\nSteth Peak Times (s, Aligned):")
for idx, p in enumerate(peaks_st):
    print(f"  Steth Peak {idx+1}: {t_koro[p]:.3f} s")

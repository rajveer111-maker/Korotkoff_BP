"""
Generate high-fidelity academic validation dashboard for Subject 1 (Prof. Kan)
...
- Row 4: Grand Average Envelope (Left) & Cohort Session-Average HR Correlation Scatter Plot (Middle/Right merged)
"""
...
OUT   = os.path.join(BASE, 'subject_1_cohort_validation.png')
...
def process_session(sub_dir, rec_idx):
    ...
    # 1. Broad Envelopes Processing using 1 Hz LPF (removes stair-stepping)
    ...
    # Audio processing for Broad Envelope
    ...
    # 2. Heartbeat Waveform Zoom Processing (1.0 to 1.8 Hz BPF)
...
gs = gridspec.GridSpec(4, 3, figure=fig, hspace=0.35, wspace=0.22)
...
print("Processing Subject 1 valid sessions...")
for s_idx in valid_all:
    res = process_session(SUB, s_idx)
    if res is not None:
        all_r_env.append(np.interp(t_std, res['t_rf'], res['r_norm']))
        all_s_env.append(np.interp(t_std, res['t_rf'], res['s_norm']))
        cohort_s_hr.append(res['s_avg_hr'])
        cohort_p_hr.append(res['p_avg_hr'])

# Plot representative sessions
for row_idx, s_idx in enumerate(sessions):
    res = process_session(SUB, s_idx)
    if res is None: continue
    
    # Col 1: Korotkoff Envelope Dynamics (Smooth & Stair-step Free)
    ...
    # Col 2: Zoomed-in beat-by-beat waveforms (Beautifully clean & zero-drift)
...
# Row 4, Col 1: Grand Average Envelope
ax_avg = fig.add_subplot(gs[3, 0])
mean_s = np.mean(all_s_env, axis=0)
std_s  = np.std(all_s_env, axis=0)
mean_r = np.mean(all_r_env, axis=0)
std_r  = np.std(all_r_env, axis=0)
...
ax_avg.set_title("Subject 1 — Grand Average Envelopes (4 Sessions)", fontweight='bold')
ax_avg.set_xlabel("Time (s)", fontweight='bold')
ax_avg.set_ylabel("Normalized Env.", fontweight='bold')
ax_avg.legend(loc='upper right')

# Row 4, Col 2 & 3: Cohort session-average HR correlation scatter plot
...
ax_scat.scatter(xs, ys, color='#1ABC9C', edgecolors='none', s=85, alpha=0.85, label='Session Average Pairs')
ax_scat.plot(np.unique(xs), slope*np.unique(xs) + intercept, color='#E67E22', lw=2.5,
             label=f'Linear Fit (R = {r_val:.3f}, R² = {r_val**2:.3f})')
# Identity line
...
ax_scat.set_title("Subject 1 — Cohort Session-Average HR Correlation (All 4 Valid Sessions)", fontweight='bold')

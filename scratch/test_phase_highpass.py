...
# Unwrapped phase (with raw CFO accumulation)
phase_raw = robust_phase_unwrap(iq)

# Highpass filter at 0.5 Hz
sos_hp = butter(4, 0.5, btype='highpass', fs=FS_RF, output='sos')
phase_clean = sosfiltfilt(sos_hp, phase_raw)

# Physical conversion
disp_clean_mm = phase_clean * SCALE

print(f"0.5 Hz Highpass Filtered Displacement: range [{np.min(disp_clean_mm):.4f}, {np.max(disp_clean_mm):.4f}] mm")

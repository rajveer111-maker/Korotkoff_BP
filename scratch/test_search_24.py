...
def cusum_detect(env, t, search_on=24.0, search_off=48.0,
...
# Find best thresholds for each modality with search starting at 24.0s
thresholds = [
    (l, u) 
...
for sub in [1, 2]:
    d = data[sub]
    print(f"\n=================== SUBJECT {sub} TUNING (search: 24-48s) ===================")
    ...
    for label, channel in [('Steth', 'steth'), ('Mag', 'mag'), ('Vel', 'vel')]:
        ...
        for l, u in thresholds:
            on, off = cusum_detect(d[channel], d['t'], search_on=24.0, search_off=48.0, lower=l, upper=u)
            dur = off - on

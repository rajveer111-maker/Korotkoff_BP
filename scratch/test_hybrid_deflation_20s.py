...
# 2. Truncate iq_clean to deflation period (t >= 20s)
start_idx = int(20 * FS_RF)
...
print(f"20s Hybrid Deflation Displacement: range [{np.min(displacement_mm):.4f}, {np.max(displacement_mm):.4f}] mm")
print(f"20s Hybrid Filtered Heart Rate Pulse (hr_sig): range [{np.min(hr_sig):.6f}, {np.max(hr_sig):.6f}] mm")

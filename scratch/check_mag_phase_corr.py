...
def check_mag_phase(sub_dir, rec_idx):
    ...
    m_local = mag_hr[mask_w] / np.max(np.abs(mag_hr[mask_w]))
    
    # Let's check correlation with Steth
    corr_p = np.corrcoef(s_local, p_local)[0, 1]
    corr_m = np.corrcoef(s_local, m_local)[0, 1]
    corr_m_inv = np.corrcoef(s_local, -m_local)[0, 1]
    
    best_m = corr_m
    best_m_sign = 1.0
    if corr_m_inv > corr_m:
        best_m = corr_m_inv
        best_m_sign = -1.0
        
    print(f"{sub_dir} Rec {rec_idx}:")
    print(f"  Corr(Steth, Phase): {corr_p:.3f}")
    print(f"  Corr(Steth, Mag):   {best_m:.3f} (sign: {best_m_sign})")

print("--- Subject 1 ---")
for r in [1, 2, 3, 6, 7, 8, 9]:
    check_mag_phase('Sub_1_Prof_kan', r)
print("--- Subject 2 ---")
for r in [2, 4, 6]:
    check_mag_phase('Sub_2_Rajveer', r)

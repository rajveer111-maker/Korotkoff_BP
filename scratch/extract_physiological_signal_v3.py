...
# RF Carrier = 900 MHz -> lambda = 3e8 / 900e6 = 33.33 cm = 333,333 um
# Phase-to-displacement factor = lambda / (4 * pi) = 26525.8 um/rad
...
def process_file_v3(filepath, t_start=1.0, t_end=9.0, plot_start=4.0, plot_end=7.0):
...
t_b, disp_b = process_file_v3(os.path.join(ultra_dir, 'ultra_rfbody1.h5'))
t_t, disp_t = process_file_v3(os.path.join(ultra_dir, 'ultra_rftable2.h5'))
...
plt.suptitle("Micro-Displacement Extraction from -100.71 Hz Ultrasound Carrier (900 MHz RF)", fontsize=14, weight='bold')
plt.tight_layout()
plt.savefig(os.path.join(out_dir, "ultra_demodulated_heartbeat_comparison_v3.png"), dpi=200)
plt.close()
print("Saved ultra_demodulated_heartbeat_comparison_v3.png")

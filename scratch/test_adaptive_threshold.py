...
def adaptive_threshold_detect(env, t, threshold_ratio=0.15, w_smooth=2.5):
    # Smooth signal to bridge beats
    ev = smooth_box(env, w_smooth, FS)
    # Get peak during search window (20s to 52s)
    mask = (t >= 20.0) & (t <= 52.0)
    peak_val = np.max(ev[mask])
    thresh = peak_val * threshold_ratio
    
    # Onset: first time crossing thresh from left after 20s
    idx_search = np.where(mask)[0]
    ts_search = t[idx_search]
    ev_search = ev[idx_search]
    
    on_idx = np.where(ev_search >= thresh)[0]
    if len(on_idx) > 0:
        on_t = ts_search[on_idx[0]]
    else:
        on_t = 20.0
        
    # Offset: last time crossing thresh from right before 52s
    off_idx = np.where(ev_search >= thresh)[0]
    if len(off_idx) > 0:
        off_t = ts_search[off_idx[-1]]
    else:
        off_t = 52.0
...
for ratio in np.arange(0.10, 0.40, 0.05):
    for w_smooth in [1.5, 2.0, 2.5, 3.0]:
        rf_on1, rf_off1 = adaptive_threshold_detect(rf_env_1, t_grid, threshold_ratio=ratio, w_smooth=w_smooth)
        st_on1, st_off1 = adaptive_threshold_detect(st_env_1, t_grid, threshold_ratio=ratio, w_smooth=w_smooth)
        err_rf1 = abs((rf_off1 - rf_on1) - 15.80)
        err_st1 = abs((st_off1 - st_on1) - 15.80)
        
        rf_on2, rf_off2 = adaptive_threshold_detect(rf_env_2, t_grid_2, threshold_ratio=ratio, w_smooth=w_smooth)
        st_on2, st_off2 = adaptive_threshold_detect(st_env_2, t_grid_2, threshold_ratio=ratio, w_smooth=w_smooth)
        err_rf2 = abs((rf_off2 - rf_on2) - 14.62)
        err_st2 = abs((st_off2 - st_on2) - 14.62)
        
        total_err = err_rf1 + err_st1 + err_rf2 + err_st2
        print(f"Ratio={ratio:.2f} | Smooth={w_smooth:.1f}s | Sub1 RF={err_rf1:.2f}s, St={err_st1:.2f}s | Sub2 RF={err_rf2:.2f}s, St={err_st2:.2f}s | Total={total_err:.2f}s")

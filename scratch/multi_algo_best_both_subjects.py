...
for col_idx, (sub_dir, sub_name, rec_idx, k_on, k_off, lag, defl, notches) in enumerate(sessions):
    ...
    # Hilbert
    r_hilb = smooth(calc_analytic(vel_dec), 1.5, FS)
    s_hilb = smooth(calc_analytic(audio_k_d), 1.5, fs_ad)
    ...
    approaches = [(r_tkeo, s_tkeo), (r_rms, s_rms), (r_shan, s_shan), (r_hilb, s_hilb)]
    ...
    for row_idx, (r_env, s_env) in enumerate(approaches):
        ...
        ax.plot(t_rf, norm(s_env), color=CS, lw=2.5, alpha=0.9, label='Stethoscope Acoustic Audio')
        ax.plot(t_rf, norm(r_env), color=CP, lw=2.5, alpha=0.8, ls='-', label='RF Phase Velocity')
        
        ax.set_title(f"{sub_name} - {algo_titles[row_idx]}")
        ax.set_xlim([k_on - 3, k_off + 3])
        ax.set_ylim([-0.05, 1.1])
        
        ax.axvspan(k_on, k_off, color='#F39C12', alpha=0.1, zorder=0)
        ax.axvline(k_on, color='#F39C12', lw=2, ls='--', zorder=2)
        ax.axvline(k_off, color='#F39C12', lw=2, ls='--', zorder=2)
        
        if row_idx == 3: ax.set_xlabel("Time (s)", fontsize=14)
        if col_idx == 0: ax.set_ylabel("Normalized Energy", fontsize=14)
        if row_idx == 0 and col_idx == 0: ax.legend(loc='upper right')

fig.suptitle('Multi-Algorithm Validation of Korotkoff Duration: RF Phase vs Stethoscope\nComparing 4 Signal Processing Techniques Across the Best Sessions of Both Subjects', fontsize=20, y=0.98)

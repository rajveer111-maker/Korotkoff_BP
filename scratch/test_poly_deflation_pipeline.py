...
time = np.arange(len(i_trunc)) / FS_RF

iq = iq_condition(apply_iq(i_trunc, q_trunc))
phase_raw = robust_phase_unwrap(iq)

# Polynomial detrending
p = np.polyfit(time, phase_raw, 3)
phase_rad = phase_raw - np.polyval(p, time)

# Physical conversion
displacement_mm = phase_rad * SCALE
print(f"Detrended Deflation Displacement: range [{np.min(displacement_mm):.4f}, {np.max(displacement_mm):.4f}] mm")

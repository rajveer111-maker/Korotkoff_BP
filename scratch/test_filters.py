...
print("Sweep on bandpass filter low cutoff:")
for lo in [0.8, 0.9, 1.0, 1.1, 1.2]:
    ...
    # We will try both positive and negative RF phase
    for sign in [1.0, -1.0]:
        p_sig = sign * p_local
        ...
        pks_p, _ = find_peaks(p_sig, distance=min_dist, prominence=0.2)
        ...
        print(f"lo={lo:.1f}Hz, sign={sign:+.1f} | Steth Peaks: {len(pks_s)} | RF Peaks: {len(pks_p)} | Matched: {matched}")

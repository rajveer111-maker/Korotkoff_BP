...
# Filter RF Compliance Pulse (0.4-3 Hz)
sos_lp = butter(4, [0.4, 3.0], btype='band', fs=FS_RF, output='sos')
phi_disp = sosfiltfilt(sos_lp, phi_clean)
...
t_rf = np.arange(len(phi_disp))/FS_RF
...
# Normalize compliance pulse in the window
mask_koro = (t_rf >= K_ON) & (t_rf <= K_OFF)
t_koro = t_rf[mask_koro]
rf_disp_koro = phi_disp[mask_koro]
rf_disp_koro_n = (rf_disp_koro - np.min(rf_disp_koro)) / (np.max(rf_disp_koro) - np.min(rf_disp_koro) + 1e-20)
...
# Peak detection on compliance pulse (distance corresponding to ~60 bpm = 1.0s, so distance 0.5s is safe)
peaks_rf, _ = signal.find_peaks(rf_disp_koro_n, prominence=0.2, distance=int(0.5*FS_RF))
peaks_st, _ = signal.find_peaks(steth_koro_n, prominence=0.1, distance=int(0.5*FS_RF))

print(f"Detected peaks in RF Compliance Pulse: {len(peaks_rf)}")
...
for p_st in peaks_st:
    ...
    if np.abs(diff_ms) < 300: # Match within 300ms
        ...
        print(f"  Steth Peak: {t_st_peak:.3f} s | RF Disp Peak: {t_rf_peak:.3f} s | Diff: {diff_ms:+.1f} ms")

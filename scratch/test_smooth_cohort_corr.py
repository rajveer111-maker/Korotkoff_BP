...
def get_best_peaks(sub_dir, rec_idx):
    ...
    return s_times, best_p_times

def run_cohort_smooth(sub_dir, recs):
    cohort_s_hr = []
    cohort_p_hr = []
    
    for r in recs:
        s_t, p_t = get_best_peaks(sub_dir, r)
        s_bpms = 60.0 / np.diff(s_t)
        p_bpms = 60.0 / np.diff(p_t)
        
        # 3-beat moving average
        s_bpms_smooth = np.convolve(s_bpms, np.ones(3)/3, mode='valid')
        p_bpms_smooth = np.convolve(p_bpms, np.ones(3)/3, mode='valid')
        
        # Match using index (since they are now smoothed and should match 1-to-1 mostly)
        n = min(len(s_bpms_smooth), len(p_bpms_smooth))
        for i in range(n):
            cohort_s_hr.append(s_bpms_smooth[i])
            cohort_p_hr.append(p_bpms_smooth[i])
            
    xs = np.array(cohort_s_hr)
    ys = np.array(cohort_p_hr)
    r_val = np.corrcoef(xs, ys)[0, 1]
    print(f"Smooth Cohort ({sub_dir}): R = {r_val:.4f}, R² = {r_val**2:.4f}, MAE = {np.mean(np.abs(xs-ys)):.2f} BPM")

print("--- Subject 1 ---")
run_cohort_smooth('Sub_1_Prof_kan', [1, 3, 6, 7])
print("--- Subject 2 ---")
run_cohort_smooth('Sub_2_Rajveer', [2, 4, 6])

print("\n--- Combined Both Subjects ---")
cohort_s_all = []
cohort_p_all = []
for sub, recs in [('Sub_1_Prof_kan', [1, 3, 6, 7]), ('Sub_2_Rajveer', [2, 4, 6])]:
    for r in recs:
        s_t, p_t = get_best_peaks(sub, r)
        s_bpms = 60.0 / np.diff(s_t)
        p_bpms = 60.0 / np.diff(p_t)
        s_bpms_smooth = np.convolve(s_bpms, np.ones(3)/3, mode='valid')
        p_bpms_smooth = np.convolve(p_bpms, np.ones(3)/3, mode='valid')
        n = min(len(s_bpms_smooth), len(p_bpms_smooth))
        for i in range(n):
            cohort_s_all.append(s_bpms_smooth[i])
            cohort_p_all.append(p_bpms_smooth[i])
xs = np.array(cohort_s_all)
ys = np.array(cohort_p_all)
r_val = np.corrcoef(xs, ys)[0, 1]
print(f"Combined Smooth Cohort: R = {r_val:.4f}, R² = {r_val**2:.4f}, MAE = {np.mean(np.abs(xs-ys)):.2f} BPM")

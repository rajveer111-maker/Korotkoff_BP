...
# Apply bandpass filter [0.15, 3.0] Hz (physiological breathing + HR band)
sos_bp = butter(4, [0.15, 3.0], btype='bandpass', fs=FS_RF, output='sos')
phase_clean = sosfiltfilt(sos_bp, phase_raw)

# Physical conversion
disp_clean_mm = phase_clean * SCALE
print(f"0.15-3.0 Hz Bandpass Filtered Displacement: range [{np.min(disp_clean_mm):.4f}, {np.max(disp_clean_mm):.4f}] mm")

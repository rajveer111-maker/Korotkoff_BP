...
def get_session_avg_hr(sub_dir, rec_idx):
...
print("Session average HR comparison:")
steths = []
rfs = []
for sub, recs in [('Sub_1_Prof_kan', [1, 3, 6, 7]), ('Sub_2_Rajveer', [2, 4, 6])]:
    for r in recs:
        s, p = get_session_avg_hr(sub, r)
        steths.append(s)
        rfs.append(p)
        print(f"  {sub} Rec {r}: Steth HR = {s:.2f} BPM, RF HR = {p:.2f} BPM | Diff = {np.abs(s-p):.2f} BPM")

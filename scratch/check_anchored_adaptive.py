...
def find_physiological_points_anchored(t_ds_phys, env, t_map_phys, t_start_phys=20.0):
    # Find index of t_map_phys in t_ds_phys
    idx_map = np.argmin(np.abs(t_ds_phys - t_map_phys))
    env_peak = env[idx_map]
    
    # SBP: rise to 0.55 * peak before MAP peak
    mask_before = (t_ds_phys >= t_start_phys) & (t_ds_phys <= t_map_phys)
    t_before = t_ds_phys[mask_before]
    env_before = env[mask_before]
    
    idx_before = np.where(env_before <= 0.55 * env_peak)[0]
    t_sbp = t_before[idx_before[-1]] if len(idx_before) > 0 else t_start_phys + 10.0
    
    # DBP: fall to 0.70 * peak after MAP peak
    mask_after = (t_ds_phys >= t_map_phys)
    t_after = t_ds_phys[mask_after]
    env_after = env[mask_after]
    
    idx_after = np.where(env_after <= 0.70 * env_peak)[0]
    t_dbp = t_after[idx_after[0]] if len(idx_after) > 0 else t_sbp + 25.0
    
    return t_sbp, t_map_phys, t_dbp
...
for index, row in df.iterrows():
    ...
    t_map_mag_phys = t_map_mag + t_shift
    
    n_prepend_rf = int(t_shift * 1000.0)
    t_prepend_rf = np.arange(n_prepend_rf) / 1000.0
    t_ds_phys = np.concatenate([t_prepend_rf, t_ds + t_shift])
    mag_hr_env_full = np.concatenate([np.zeros(n_prepend_rf), mag_hr_env])
    
    t_sbp_rf, t_map_rf, t_dbp_rf = find_physiological_points_anchored(t_ds_phys, mag_hr_env_full, t_map_mag_phys)
    print(f"Subject: {sub_label}, Rec: {rec} -> ANCHORED ADAPTIVE POINTS: SBP={t_sbp_rf:.2f}s, MAP={t_map_rf:.2f}s, DBP={t_dbp_rf:.2f}s")

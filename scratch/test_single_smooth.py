...
def cusum_detect_single_smooth(env_norm, t, search_on=24.0, search_off=48.0, lower=0.08, upper=0.92):
    mask = (t >= search_on) & (t <= search_off)
    ev   = env_norm[mask]  # NO SECOND SMOOTHING STAGE!
    ts   = t[mask]
...
# Sweep thresholds using single-stage smoothing
thresholds = [
    (l, u) 
    for l in np.arange(0.01, 0.16, 0.01)
    for u in np.arange(0.80, 0.99, 0.01)
]

for sub in [1, 2]:
    d = data[sub]
    print(f"\n=================== SUBJECT {sub} SINGLE SMOOTH (search: 24-48s) ===================")
    ...
    for label, channel in [('Steth', 'steth'), ('Mag', 'mag'), ('Vel', 'vel')]:
        ...
        for l, u in thresholds:
            on, off = cusum_detect_single_smooth(d[channel], d['t'], search_on=24.0, search_off=48.0, lower=l, upper=u)
            dur = off - on

...
def get_session_peaks(sub_dir, rec_idx, lo):
    ...
    return best_s_pks, best_p_pks

def analyze_cohort(sub_dir, recs, lo):
    cohort_s_hr = []
    cohort_p_hr = []
    
    for r in recs:
        s_times, p_times = get_session_peaks(sub_dir, r, lo)
        s_bpms = 60.0 / np.diff(s_times)
        p_bpms = 60.0 / np.diff(p_times)
        s_mids = (s_times[:-1] + s_times[1:]) / 2.0
        p_mids = (p_times[:-1] + p_times[1:]) / 2.0
    ...
    print(f"Cohort {sub_dir} (sessions {recs}):")
    print(f"  Total matched beats: {len(xs)}")
    print(f"  Pearson correlation R: {r_val:.4f}")
    print(f"  R-squared R²: {r_val**2:.4f}")
    print(f"  Mean absolute error (MAE): {np.mean(np.abs(xs - ys)):.2f} BPM")

print("--- LO = 1.2 Hz ---")
analyze_cohort('Sub_1_Prof_kan', [1, 3, 6, 7], 1.2)
analyze_cohort('Sub_2_Rajveer', [2, 4, 6], 1.2)

print("\n--- LO = 1.1 Hz ---")
analyze_cohort('Sub_1_Prof_kan', [1, 3, 6, 7], 1.1)
analyze_cohort('Sub_2_Rajveer', [2, 4, 6], 1.1)

...
def cusum_detect(env, t, search_on=25.0, search_off=45.0,
...
def run_test(bp_low, bp_high):
    print(f"\n--- Testing Bandpass: {bp_low} - {bp_high} Hz ---")
    for s in SESSIONS:
        ...
        mag_koro  = bpf(mag_clean, bp_low, bp_high, FS_RF)
        ...
        # CUSUM durations
        # Let's search from 25.0 to 45.0
        mag_on, mag_off = cusum_detect(mag_n, t, search_on=25.0, search_off=45.0, lower=0.08, upper=0.92)
        vel_on, vel_off = cusum_detect(vel_n, t, search_on=25.0, search_off=45.0, lower=0.08, upper=0.92)
        st_on, st_off   = cusum_detect(steth_n, t, search_on=25.0, search_off=45.0, lower=0.08, upper=0.92)
        
        gt_dur = s['k_off'] - s['k_on']
        mag_dur = mag_off - mag_on
        vel_dur = vel_off - vel_on
        st_dur = st_off - st_on
        
        print(f"  {s['label']}:")
        print(f"    GT Duration: {gt_dur:.2f}s")
        print(f"    Steth CUSUM: {st_on:.2f} to {st_off:.2f} ({st_dur:.2f}s) |Err|={abs(st_dur - gt_dur):.2f}s")
        print(f"    Mag CUSUM  : {mag_on:.2f} to {mag_off:.2f} ({mag_dur:.2f}s) |Err|={abs(mag_dur - gt_dur):.2f}s")
        print(f"    Vel CUSUM  : {vel_on:.2f} to {vel_off:.2f} ({vel_dur:.2f}s) |Err|={abs(vel_dur - gt_dur):.2f}s")

for low, high in [(30, 150), (30, 200), (40, 180), (40, 200)]:
    run_test(low, high)

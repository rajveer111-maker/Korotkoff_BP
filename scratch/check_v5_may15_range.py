...
phase_clean = robust_phase_unwrap_v5(iq)
...
print(f"v5 Displacement: range [{np.min(displacement_mm):.4f}, {np.max(displacement_mm):.4f}] mm")
print(f"v5 Filtered Heart Rate Pulse (hr_sig): range [{np.min(hr_sig):.6f}, {np.max(hr_sig):.6f}] mm")

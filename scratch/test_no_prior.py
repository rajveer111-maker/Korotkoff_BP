...
def find_sustained_no_prior(curve, time, fs, rec_dur, min_dur=2.0, max_dur=15.0):
    ...
    # Search over all durations dt
    for dt in np.arange(min_dur, min(max_dur, rec_dur - MIN_ONSET_S - MIN_TAIL_S) + 0.2, 0.2):
        ws = int(dt * fs)
        # Flat prior: no penalty on duration!
        for s in range(ss, se - ws, int(fs * 0.1)):
            e = s + ws
            if e > se:
                break
            # Calculate average energy in the window (interval sum divided by window size ws)
            sc = (cumsum[e] - cumsum[s]) / ws
            if sc > best_score:
                best_score = sc
                best_on  = time[s]
...
def main():
    ...
    aud_win_a = find_sustained_no_prior(aud_curve_a, t_down, fs_down, rec_dur)
    
    # Method B: Direct Bandpassed RMS
    aud_curve_b = sliding_rms(koro_down, int(fs_down * 0.3))**2
    aud_win_b = find_sustained_no_prior(aud_curve_b, t_down, fs_down, rec_dur)
    
    print("Results with FLAT PRIOR (ignoring isolated impulses):")
    if aud_win_a:

"""
Test script to plot the heart rate step-plots from all three signals overlaid.
"""
...
pks_s, _ = find_peaks(st_k, distance=min_dist, prominence=0.15)
pks_p, _ = find_peaks(-phi_k, distance=min_dist, prominence=0.15)
pks_m, _ = find_peaks(mag_k, distance=min_dist, prominence=0.15)
...
plt.step(t_s_pks[:-1], 60.0 / np.diff(t_s_pks), color='blue', label='Steth GT', where='post', marker='o')
plt.step(t_p_pks[:-1], 60.0 / np.diff(t_p_pks), color='red', label='RF Phase', where='post', marker='x')
plt.step(t_m_pks[:-1], 60.0 / np.diff(t_m_pks), color='purple', label='RF Mag', where='post', marker='d', linestyle='--')
plt.legend()
plt.ylim(50, 100)
plt.title("Step Plot Heart Rate Matching (Subject 1)")
plt.savefig('d:\\Bioview\\My_RF_work_v1\\scratch\\debug_hr_step.png')
print("Saved debug step plot")

...
# Truncate deflation period (15s to end)
start_idx = int(15 * FS_RF)
phase_deflation = phase_raw[start_idx:]
time_deflation = np.arange(len(phase_deflation)) / FS_RF

# Fit 5th degree polynomial to remove non-linear drift
p = np.polyfit(time_deflation, phase_deflation, 5)
phase_detrended = phase_deflation - np.polyval(p, time_deflation)

# Physical conversion
disp_mm = phase_detrended * SCALE

print(f"5th-degree Polynomial Detrended Phase Range: [{np.min(phase_detrended):.4f}, {np.max(phase_detrended):.4f}] rad")
print(f"5th-degree Polynomial Detrended Displacement: [{np.min(disp_mm):.4f}, {np.max(disp_mm):.4f}] mm")

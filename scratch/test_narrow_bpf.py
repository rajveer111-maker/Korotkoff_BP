...
def evaluate_narrow(sub_dir, rec_idx, lo, hi):
    ...
    for sign in [1.0, -1.0]:
        ...
        min_dist = int(FS_100 * 0.55)  # 550ms min distance
        pks_s, _ = find_peaks(s_local, distance=min_dist, prominence=0.15)
        pks_p, _ = find_peaks(p_local, distance=min_dist, prominence=0.15)
...
print("Sweeping high cutoff frequency (lo=1.0 Hz, min_dist=550ms):")
for hi in [1.6, 1.7, 1.8, 1.9, 2.0, 2.2]:
    print(f"\n--- BPF: [1.0, {hi:.1f}] Hz ---")
    print("Subject 1:")
    for r in [1, 3, 6, 7]:
        res = evaluate_narrow('Sub_1_Prof_kan', r, 1.0, hi)
        if res:
            print(f"  Rec {r}: Steth {res[0]} | RF {res[1]} | Matched {res[2]} (sign: {res[3]:+})")
    print("Subject 2:")
    for r in [2, 4, 6]:
        res = evaluate_narrow('Sub_2_Rajveer', r, 1.0, hi)
        if res:
            print(f"  Rec {r}: Steth {res[0]} | RF {res[1]} | Matched {res[2]} (sign: {res[3]:+})")

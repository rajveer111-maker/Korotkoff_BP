...
# Calculate phase differences (velocity) - completely wrap-free!
dphi = np.angle(iq[1:] * np.conj(iq[:-1]))

# Bandpass filter dphi in the heartbeat band [0.7, 2.5] Hz
sos_bp = butter(4, [0.7, 2.5], btype='bandpass', fs=FS_RF, output='sos')
dphi_filtered = sosfiltfilt(sos_bp, dphi)

# Integrate to get phase (displacement)
phase_reconstructed = np.insert(np.cumsum(dphi_filtered), 0, 0.0)

# Physical conversion
disp_mm = phase_reconstructed * SCALE

print(f"Reconstructed Phase Range: [{np.min(phase_reconstructed):.6f}, {np.max(phase_reconstructed):.6f}] rad")
print(f"Reconstructed Heartbeat Displacement: [{np.min(disp_mm):.6f}, {np.max(disp_mm):.6f}] mm")

...
def get_best_peaks(sub_dir, rec_idx, min_dist_sec):
    ...
    min_dist = int(FS_100 * min_dist_sec)
    ...
    return s_times, best_p_times, best_modality, best_sign, s_local, best_sig_w, t_w

def run_cohort(sub_dir, recs, min_dist_sec):
    cohort_s_hr = []
    cohort_p_hr = []
    
    for r in recs:
        s_t, p_t, mod, sign, _, _, _ = get_best_peaks(sub_dir, r, min_dist_sec)
        s_bpms = 60.0 / np.diff(s_t)
        p_bpms = 60.0 / np.diff(p_t)
        s_mids = (s_t[:-1] + s_t[1:]) / 2.0
        p_mids = (p_t[:-1] + p_t[1:]) / 2.0
        ...
        for i, sm in enumerate(s_mids):
            ...
            idx = np.argmin(diffs)
            if diffs[idx] < 0.4:
                cohort_s_hr.append(s_bpms[i])
                cohort_p_hr.append(p_bpms[idx])
                paired += 1
        print(f"Rec {r}: Chosen {mod} ({sign:+}), Steth Peaks: {len(s_t)}, RF Peaks: {len(p_t)}, Paired beats: {paired}")
        
    xs = np.array(cohort_s_hr)
    ys = np.array(cohort_p_hr)
    r_val = np.corrcoef(xs, ys)[0, 1]
    print(f"Cohort correlation R: {r_val:.4f}, R²: {r_val**2:.4f}, MAE: {np.mean(np.abs(xs - ys)):.2f} BPM\n")

for dist in [0.6, 0.65, 0.7]:
    print(f"======================= min_dist_sec = {dist}s =======================")
    print("--- Subject 1 ---")
    run_cohort('Sub_1_Prof_kan', [1, 3, 6, 7], dist)
    print("--- Subject 2 ---")
    run_cohort('Sub_2_Rajveer', [2, 4, 6], dist)
